...
class New extends React.Component {
	...
	_fetchProjects() {
		...
		axios.get(window.location.origin + '/api/project/user').then(response => {
		...
		return projects.map((project, index) => {
			return (
				<a
					...
					key={index}
					href={'/project?p=' + project.title}
				>
					<h2>{project.title}</h2>
					<a
						className="btnDelete"
						href=""
						onClick={this._deleteProject.bind({
							fetch: this._fetchProjects.bind(this),
							name: project.title,
						})}
					>
	...
	_newProject(event) {
		...
		if (!projectName) {
			//if project name was not provided
			this.setState({
				errorMsg: 'No project name provided',
			});
			errorDiv.style.display = 'block';
		} else if (!validName.test(projectName)) {
			//if project name provided but formatted incorrectly
			this.setState({
			...
			axios
				.post(window.location.origin + '/api/project', { projectName })
				.then(() => {
		...
		return (
			<div id="newSection">
				...
				<a
					...
					className="btn"
					id="newProject"
					onClick={e => this._newProject(e)}
				>
					Start New Project
				</a>
				<div id="nameError">{this.state.errorMsg}</div>

				<div id="existingProjects">{this._populateProjects()}</div>
			</div>
		);
	}
	//====//
